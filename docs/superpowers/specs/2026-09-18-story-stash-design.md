...
**Türen/Räume**
- Keller-Tür `hinterzimmer` ist eine normale Tür in `Story.DOORS`: sichtbar, wenn `story.enabled.hinterzimmer` gesetzt ist **oder** die Tür in `unlocked.doors` steht; öffnbar nur mit `unlock: { doors: ['hinterzimmer'] }` (`enable` zeigt sie vorher als gesperrt, z. B. „Anabis Tisch kommt morgen"). Die Spiel-ID, die an `Game.settle`/`Bus.emit('stake', …)` geht, heißt `baccarat` (darauf zielt `turnoverGroups`); Tür-ID und Spiel-ID sind bewusst unterschiedlich. Im freien Spiel nicht vorhanden (die Tür-Sperrprüfung greift nur im Story-Modus).
